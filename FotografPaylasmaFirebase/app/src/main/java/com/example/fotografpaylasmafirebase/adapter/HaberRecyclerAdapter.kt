...
class HaberRecyclerAdapter(val postList : ArrayList<Post>) : RecyclerView.Adapter<HaberRecyclerAdapter.PostHolder>() {
    ...
    override fun onBindViewHolder(holder: PostHolder, position: Int) {
        val textViewEmail = holder.itemView.findViewById<TextView>(R.id.recycler_row_kullanici_email)
        textViewEmail.text = postList[position].kullaniciEmail
        val textViewYorum = holder.itemView.findViewById<TextView>(R.id.recycler_row_yorum_text)
        textViewYorum.text = postList[position].kullaniciYorum
        val imageview = holder.itemView.findViewById<ImageView>(R.id.recycler_row_imageview)
        Picasso.get().load(postList[position].gorselUrl).into(imageview)
    }
